...
module.exports = {
  create: async function (req, res) {
    ...
    await Users.create({ login: `admin${provider.id}`, password: `password${provider.id}`, role: Role.LocalAdmin, provider: provider.id }).fetch();
    await Settings.create({ provider: provider.id }).fetch();
    await Instructors.create({ name: 'Тренер по-умолчанию', color: '#00e1ff', provider: provider.id }).fetch();
