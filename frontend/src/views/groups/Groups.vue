...
export default {
  ...
  methods: {
    ...
    async fetchSettings() {
      ...
      this.itemForm[3].models = this.persons;
      this.itemForm[4].models = this.places;
    ...
    async fetchGroups() {
      this.groups = await this.$getAsync(this.baseUrl, {
        type: GroupType.General,
        defaultInstructor: this.selectedInstructor,
        sort: "updatedAt DESC"
      });
    },
    ...
    showAddModal() {
      this.itemForm[3].value = this.selectedInstructor;
      this.itemForm[6].value = GroupType.General;
    ...
    goToDetailPage(group) {
      this.$router.push({ 
        path: `/cp/groups/${group.id}`
      });
    },
    getScheduleView(value){
      if (!value) 
        return "";
      const weekdays = this.$moment.weekdaysMin();
